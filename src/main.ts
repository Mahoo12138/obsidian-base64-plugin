...
class Base64ImageModal extends Modal {
    ...
    onOpen() {
        ...
        // 标题
        contentEl.createEl('h2', { text: 'Base64 图片管理' });
        ...
        const size = Math.round(this.info.dataUrl.length / 1024);
        const format = this.info.dataUrl.match(/data:image\/(\w+);/)?.[1] || '未知';
        ...
        saveBtn.style.gridColumn = '1 / -1'; // 占据整行
        saveBtn.addEventListener('click', () => {
            this.saveChanges(altInput.value, currentBase64);
        });

        // 转换为本地文件按钮
        const convertBtn = buttonSection.createEl('button', { text: '📁 转为本地文件' });
        convertBtn.addEventListener('click', () => {
            this.convertToLocalFile();
        });

        // 复制 Base64 按钮
        const copyBtn = buttonSection.createEl('button', { text: '📋 复制 Base64' });
        ...
        // 下载图片按钮
        const downloadBtn = buttonSection.createEl('button', { text: '⬇️ 下载图片' });
        downloadBtn.addEventListener('click', () => {
            this.downloadImage();
        });

        // 删除按钮
        const deleteBtn = buttonSection.createEl('button', { text: '🗑️ 删除图片', cls: 'mod-warning' });
        deleteBtn.addEventListener('click', () => {
            this.deleteImage();
        });

        // 关闭按钮
        const closeBtn = buttonSection.createEl('button', { text: '❌ 关闭' });
